...
X = datset_NamingAddress.iloc[:, [2,3,4,5,6,7,11]].values
y = datset_NamingAddress.iloc[:,1]

X_submission = datset_submission_NamingAddress.iloc[:, [1,2,3,4,5,6,10]].values
index_list = []
...
X[:, 6] = labelencoder_X_Embark.fit_transform(X[:, 6])
...
X_submission[:, 6] = labelencoder_X_submission_Embark.fit_transform(X_submission[:, 6])
...
onehotencoder_X = OneHotEncoder(categorical_features = [0,1,2,6])
X = onehotencoder_X.fit_transform(X).toarray()

onehotencoder_X_submission = OneHotEncoder(categorical_features = [0,1,2,6])
